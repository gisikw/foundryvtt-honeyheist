 ...
 * @extends {ActorSheet}
 */
class HoneyHeistActorSheet extends ActorSheet {
  /** @override */
  ...
  activateListeners(html) {
    ...
    html.find('.stat-button').click(ev => {
      const isBearRoll = HoneyHeistActorSheet._isBearRoll(ev.currentTarget);
    ...
    html.find('.stat-roll-single, .stat-roll-double').click(ev => {
      ...
      const rollSuccess = game.i18n.localize(
        isSuccess ? 'HH.Success' : 'HH.Failed'
      );
      const actionMessage = game.i18n.localize(
        isSuccess ? 'HH.GreedMessage' : 'HH.FrustrationMessage'
      );
      const chatMessage = `${game.i18n.localize(
        isBearRoll ? 'HH.RollForBear' : 'HH.RollForCriminal'
  ...
  _updateStats(bearOffest, roll, isBearRoll) {
    const bearStat = $('#stat-bear')
      .find('.stat-value')
      .get(0);
    const criminalStat = $('#stat-criminal')
      .find('.stat-value')
      .get(0);
    let bearVal = parseInt(bearStat.value, 10);
    let criminalVal = parseInt(criminalStat.value, 10);
    let endResult =
      (isBearRoll && bearVal === 6) || (!isBearRoll && criminalVal === 6);
  ...
  static _isBearRoll(element) {
    return element.parentElement.id === 'stat-bear';
  }
}

export default HoneyHeistActorSheet;
